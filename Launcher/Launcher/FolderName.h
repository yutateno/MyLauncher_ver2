...
private:
	...
	std::vector<std::vector<std::string>> drawname;	// 画像の名前
	...
	void SearchDrawFile(std::string gamePath, std::string gamefilename, std::string extension, int nowlist);	// 画像用の(ランチャーのためのゲームのメディア類の名前をそれぞれ調べる)
...
public:
	...
	std::vector<std::vector<std::string>> GetDrawListName();	// ゲームの画像群
